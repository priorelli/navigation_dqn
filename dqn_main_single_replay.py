...
def run_episode(episode, q_primary):
    ...
    while not episode_done and step < param.steps:
        ...
        if batch == param.max_batch - 1 or step == param.steps - 1 or episode_done:
            loss = dhl.update_network_single_replay(q_primary, experience)
...
def main():
    ...
    for episode in range(param.episodes):
        ...
        if episode % 200 == 0:
            ...
            pickle.dump(param.loss_of_episodes, open(res_folder + 'loss_of_episodes_single_replay_%d.pkl'
                        % episode, 'wb'))
            pickle.dump(param.reward_of_episodes, open(res_folder + 'reward_of_episodes_single_replay_%d.pkl'
                        % episode, 'wb'))
            pickle.dump(param.step_of_episodes, open(res_folder + 'step_of_episodes_single_replay_%d.pkl'
                        % episode, 'wb'))
            pickle.dump(param.target_scores, open(res_folder + 'target_scores_single_replay_%d.pkl'
                        % episode, 'wb'))
            pickle.dump(param.reward_visits, open(res_folder + 'reward_visits_single_replay_%d.pkl'
                        % episode, 'wb'))

            # save the network
            pickle.dump(q_primary.weights, open(res_folder + 'weights_single_replay_%d.pkl'
                        % episode, 'wb'))
            pickle.dump(q_primary.biases, open(res_folder + 'biases_single_replay_%d.pkl'
                        % episode, 'wb'))
