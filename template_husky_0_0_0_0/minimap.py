...
@nrp.MapRobotPublisher('tderror', Topic('/tderror', sensor_msgs.msg.Image))
def print_map(t, position, minimap, scores, tderror):
    import cv2
    import rospy
    ...
    cv_img = cv2.imread('/home/spock/PycharmProjects/project/plots/tderror_%d.png' % i)
    if cv_img is not None:
        msg_frame = CvBridge().cv2_to_imgmsg(cv_img, 'rgb8')
        tderror.send_message(msg_frame)
